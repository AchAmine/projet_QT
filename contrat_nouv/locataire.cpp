...
locataire::locataire(QString nom,QString prenom,QString email,QString adr,long NumTel,long CIN,int id_local)
{
    this->nom=nom ;
    this->prenom=prenom;
    this->email=email ;
...
QString NumTel1= QString::number(NumTel);
...
query.bindValue(":NumTel", NumTel1);
...
bool locataire::supprimer_locataire(long CINN)
{
    QSqlQuery query;
    QString CIN1= QString::number(CINN);
    query.prepare("Delete from locataire where CIN =:CIN ");
    query.bindValue(":CIN", CIN1);
    return    query.exec();
}

bool locataire::modifier_locataire(long CINN)
{
    QSqlQuery query ;
    QString CIN0 = QString::number(CIN);
    QString CIN1 = QString::number(CINN);
    QString NumTel1 = QString::number(NumTel);
    QString id_local1= QString::number(id_local);
...
QSqlQueryModel *locataire::afficherlct()
{QSqlQueryModel * model= new QSqlQueryModel();

model->setQuery("select * from locataire ORDER BY Nom   ");
model->setHeaderData(0, Qt::Horizontal, QObject::tr("CIN"));
model->setHeaderData(1, Qt::Horizontal, QObject::tr("Nom"));
model->setHeaderData(2, Qt::Horizontal, QObject::tr("Prenom"));
model->setHeaderData(3, Qt::Horizontal, QObject::tr("Email"));
